...
1. 打开本仓库的 [Releases 页面](../../releases)，下载最新版 APK（免登录直链）；
...
**关于签名**：自 v0.3.0 起使用专用签名密钥（30 年有效期，PKCS12，托管于 GitHub Actions Secrets，密钥本体永不入库），此后版本可覆盖安装升级。注意：**v0.2.0 及更早版本使用 CI 临时 debug 签名，与 v0.3.0 签名不同**——从旧版升级到 v0.3.0 必须先在系统设置移除本应用的设备管理员身份、卸载重装；v0.3.x 之间可直接覆盖安装。签名密钥备份在维护者本机的仓库外目录（`~/QuickAppGuard-keys/`），丢失将无法再发布升级包。
...
分层覆盖率（"能装上"与"能停用"是两个不同的层）：

| 系统 | 安卓框架 | 快应用引擎 | 处置路径 | 置信度 |
|------|----------|-----------|----------|--------|
| 小米 MIUI 12–14 / 澎湃OS 1/2 | ✔ | `com.miui.hybrid` | 全通道；HyperOS 禁用被拒自动退级挂起 | 已确认 |
| 小米澎湃OS 3（Android 16 底座） | ✔ | 同上（改名有启发式兜底） | 同上 | 待实测 |
| OPPO/一加/realme ColorOS 12–16 | ✔ | `com.nearme.instant.platform` | 全通道 | 官方文档确认 |
| vivo/iQOO OriginOS 3/4/5、旧 Funtouch | ✔ | `com.vivo.hybrid` | 全通道 | vivo 官方 Wiki 确认 |
| 华为 HarmonyOS 2/3/4 | ✔(AOSP 底座) | 内置于应用市场 | 仅官方开关引导（耦合） | 已确认 |
| 荣耀 MagicOS 8/9 | ✔ | 内置于应用市场 | 仅官方开关引导（耦合） | 已确认 |
| **华为 HarmonyOS NEXT / 5.x / 6 / 7** | ✘ | — | **不支持：纯血鸿蒙无法安装任何 APK** | 确定性结论 |
| 三星 / Pixel / 原生安卓 | ✔ | 无快应用 | 状态直接为绿，零处理 | 确定性结论 |
| 魅族/中兴/努比亚/联想 | ✔ | 候选包名 | 启发式识别 + 全通道 | 待实测回填 |
| Android 6.0 及更早 | 部分 | — | `scripts/quick-guard.sh`（adb 应急脚本） | 已确认 |

不同层的覆盖差异：检测（Layer 1）与无障碍兜底（Layer 2）在所有安卓机上可用；Shizuku（Layer 3）在 Android 11+ 免电脑；Device Owner（Layer 4）绝大多数品牌可用，个别 OEM 需先退出账号或有额外限制，以真机实测为准。

**替代方案说明**：纯血鸿蒙机型本身无法侧载 APK，家属可直接在"设置 → 快应用管理"类入口按其官方开关关闭；这也说明本项目始终以"安卓框架"为明确边界，不做无法兑现的承诺。
...
2. **上下文白名单（已实现）**：无障碍兜底从"见快应用窗口就拦"升级为"只拦从其他应用里弹出的"。判断逻辑：前一个前台窗口是桌面（通过 `CATEGORY_HOME` 动态识别，不限品牌）、负一屏/助手（内置小米/OPPO 常见包名）或快应用生态自身界面（快应用中心）→ 视为用户主动使用，放行；从浏览器/小说/视频等应用中突然弹出 → 拦截。拦截后清空上下文，防止同一广告链的下一个窗口被误判为主动使用。**局限**：负一屏包名各厂商不一，未覆盖的品牌可能误拦负一屏卡片（有 Toast 提示，可关闭兜底开关）；**已知取舍**：紧跟桌面动作后弹出的窗口会放行（桌面图标必须可点），主拦截目标是应用内弹出的广告链；广告组件窗口与「拦截厂商广告组件」实验开关联动，开关关闭时绝不拦截。
